...
const proceedDelete = () => {
  if (selectedabsensi.value) {
    window.location.href = route('absenxi.hapus', selectedabsensi.value.id_absensi)
  }
}

// Function search
const performSearch = () => {
  router.get(route('absenxi'), { search: search.value }, { preserveState: true, replace: true })
}
</script>

<template>
  <Head title="Data Absensi Kelas XI" />
              ...
              <tr v-for="ab in props.absen.data" :key="ab.id_absensi"
                ...
                <td class="px-6 py-4 text-center">
                  <div class="flex justify-center gap-2">
                    <TextLink
                      :href="route('absenxi.edit', ab.id_absensi)"
