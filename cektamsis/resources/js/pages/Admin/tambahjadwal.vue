<script setup lang="ts">
    ...
    const props = defineProps < {
        guru: {
            id_guru: number;nama: string
        } []
        mapel: {
            id_mapel: number;nama_mapel: string
        } []
        kelas: {
            id_kelas: number;nama_kelas: string
        } []
    } > ()
                ...
                <form @submit.prevent="form.post('/jadwal/tambahstore')" class="space-y-5">
                    ...
                    <div>
                        ...
                        <select v-model="form.id_guru"
                            ...
                            <option v-for="g in props.guru" :key="g.id_guru" :value="g.id_guru.toString()">
                                {{ g . nama }}
                            </option>
                        </select>
                    ...
                    <div>
                        ...
                        <select v-model="form.id_kelas"
                            ...
                            <option v-for="k in props.kelas" :key="k.id_kelas" :value="k.id_kelas.toString()">
                                {{ k . nama_kelas }}
                            </option>
                        </select>
                    ...
                    <div>
                        ...
                        <input v-model="form.lantai" type="text"
                            class="w-full rounded-lg border border-zinc-700 bg-zinc-800 text-white px-4 py-2 focus:ring focus:ring-green-500 focus:outline-none" />
                        <span v-if="form.errors.lantai" class="text-red-500 text-sm">
                            {{ form . errors . lantai }}
                        </span>
                    </div>
                    ...
                    <!-- Ruang -->
                    <div>
                        <label class="block text-sm font-medium text-zinc-300 mb-1">Ruang</label>
                        <input v-model="form.ruang" type="text"
                            class="w-full rounded-lg border border-zinc-700 bg-zinc-800 text-white px-4 py-2 focus:ring focus:ring-green-500 focus:outline-none" />
                        <span v-if="form.errors.ruang" class="text-red-500 text-sm">
                            {{ form . errors . ruang }}
                        </span>
                    </div>
                    <!-- Jam Mulai -->
