<script setup lang="ts">
    ...
    // ✅ PERBAIKAN: Perhitungan persentase dengan hadir + terlambat
    ...
    const getStatusColor = (p: number) => p >= 90 ? 'text-green-400' : p >= 80 ? 'text-yellow-400' : p >= 70 ?
        'text-orange-400' : 'text-red-400';
    const getProgressColor = (p: number) => p >= 90 ? 'bg-green-500' : p >= 80 ? 'bg-yellow-500' : p >= 70 ?
        'bg-orange-500' : 'bg-red-500';

   // ✅ PERBAIKAN: Ambil dari props yang sudah di-filter per hari (dari controller)
    const totalDistribusi = computed(() => {
        return {
            totalHadir: props.totalabsen || 0,
            totalTerlambat: props.totalterlambat || 0,
            totalIzin: props.totalizin || 0,
            totalSakit: props.totalsakit || 0,
            totalAlfa: props.totalalfa || 0,
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full flex-1 flex-col gap-6 p-6 overflow-x-auto">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <div class="flex items-center gap-3">
                            <select v-model="selectedYear"
                                class="bg-zinc-800 text-white text-sm rounded-lg px-3 py-1.5 border border-zinc-700">
                ...
                <div class="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6 shadow-lg">
                    ...
                    <div class="space-y-4">
                        <button onclick="window.location.href='/user/tambah'"
                            class="flex justify-between items-center w-full py-3 border-b border-zinc-800 text-zinc-300 hover:text-blue-300">
            ...
            <div class="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6 shadow-lg">
                ...
                <div class="space-y-4 max-h-80 overflow-y-auto custom-scrollbar">
                    <div v-for="kelas in kelasData" :key="kelas.nama_kelas" class="p-4 bg-zinc-800/40 rounded-lg">
                        ...
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span :class="getStatusColor(getPercentage(kelas))"
                        ...
                        <div class="w-full bg-zinc-700/50 rounded-full h-2 mb-3">
                            <div :class="getProgressColor(getPercentage(kelas))"
                                class="h-2 rounded-full"
                                :style="{ width: getPercentage(kelas) + '%' }"></div>
                        </div>

                        <!-- ✅ PERBAIKAN: Tambahkan kolom Terlambat, grid menjadi 5 -->
                        <div class="grid grid-cols-5 gap-2 text-xs">
                            ...
                            <div class="text-center">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <span class="text-zinc-400">Terlambat</span>
                                </div>
                                <div class="text-white font-medium mt-1">{{ kelas . terlambat }}</div>
                            </div>
                ...
                <!-- ✅ SUMMARY FIXED: Menggunakan v-if untuk memastikan data sudah ready -->
                <div v-if="kelasData.length > 0" class="mt-6 pt-4 border-t border-zinc-700">
                    <div class="grid grid-cols-4 gap-4 text-sm mb-4">

                        <div class="text-center p-3 bg-zinc-800/40 rounded-lg">
                            <div class="text-zinc-400 text-xs mb-1">Total Terlambat</div>
                            <div class="text-2xl font-bold text-orange-400">
                                {{ totalDistribusi.totalTerlambat || 0 }}
                            </div>
                        </div>

                        <div class="text-center p-3 bg-zinc-800/40 rounded-lg">
                            <div class="text-zinc-400 text-xs mb-1">Total Izin</div>
                            <div class="text-2xl font-bold text-blue-400">
                                {{ totalDistribusi.totalIzin || 0 }}
                            </div>
                        </div>

                        <div class="text-center p-3 bg-zinc-800/40 rounded-lg">
                            <div class="text-zinc-400 text-xs mb-1">Total Sakit</div>
                            <div class="text-2xl font-bold text-yellow-400">
                                {{ totalDistribusi.totalSakit || 0 }}
                            </div>
                        </div>

                        <div class="text-center p-3 bg-zinc-800/40 rounded-lg">
                            <div class="text-zinc-400 text-xs mb-1">Total Alfa</div>
                            <div class="text-2xl font-bold text-red-400">
                                {{ totalDistribusi.totalAlfa || 0 }}
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div class="text-center p-3 bg-zinc-800/40 rounded-lg">
                            <div class="text-zinc-400 text-xs mb-1">Total Kehadiran</div>
                            <div class="text-2xl font-bold text-green-400">
                                {{ totalDistribusi.totalHadir || 0 }}
                            </div>
                        </div>

                        <div class="text-center p-3 bg-zinc-800/40 rounded-lg">
                            <div class="text-zinc-400 text-xs mb-1">Total Siswa</div>
                            <div class="text-2xl font-bold text-white">
                                {{ totalDistribusi.totalSemua || 0 }}
                            </div>
                        </div>
                ...
                <div v-else class="mt-6 pt-4 border-t border-zinc-700 text-center text-zinc-400">
                    Tidak ada data distribusi kelas
                </div>
