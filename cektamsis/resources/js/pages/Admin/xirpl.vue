...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: 'Data Siswa XI RPL', href: '/siswaxi' },
];
...
const proceedDelete = () => {
  if (selectedSiswa.value) {
    window.location.href = route('siswaxi.hapus', selectedSiswa.value.id_siswa)
  }
}
...
const filterKelas = (kelas: string) => {
  router.get(route('siswaxi'), { kelas, search: searchQuery.value }, { preserveState: true })
}

// Function search
const searchData = () => {
  router.get(route('siswaxi'), { search: searchQuery.value, kelas: props.selectedKelas }, { preserveState: true })
}
</script>
