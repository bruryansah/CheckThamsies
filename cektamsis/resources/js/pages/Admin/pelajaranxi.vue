...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: 'Data Absen Pelajaran Kelas XI', href: '/pelajaranxi' },
];
...
const proceedDelete = () => {
  if (selectedabsensi.value) {
    window.location.href = route('pelajaranxi.hapus', selectedabsensi.value.id_absensi_pelajaran)
  }
}

// Function search
const performSearch = () => {
  router.get(route('pelajaranxi'), { search: search.value }, { preserveState: true})
}
</script>

<template>
  <Head title="Data Absen Pelajaran Kelas XI" />
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-6 overflow-x-auto">
      <div class="rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-lg">
        ...
        <div class="flex items-center justify-between mb-6">
          <h1 class="text-xl font-semibold text-white">Data Absen Pelajaran Kelas XI</h1>
        </div>
              ...
              <tr v-for="ab in props.absen.data" :key="ab.id_absensi_pelajaran"
                ...
                <td class="px-6 py-4 text-center">
                  <div class="flex justify-center gap-2">
                    <TextLink
                      :href="route('pelajaranxi.edit', ab.id_absensi_pelajaran)"
