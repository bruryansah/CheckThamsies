<script setup lang="ts">
    ...
    const props = defineProps < {
        jurusan: {
            id_jurusan: number; // Pastikan menggunakan id_siswa sesuai database
            nama_jurusan: string;
        },
    } > ()
    ...
    onMounted(() => {
        if (props.jurusan) {
            form.nama_jurusan = props.jurusan.nama_jurusan || '';
        }
    });
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-6 overflow-x-auto">
            <div class="rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-lg max-w-2xl w-full mx-auto">
                ...
                <!-- Form -->
                <form @submit.prevent="form.put(`/jurusan/update/${props.jurusan.id_jurusan}`)" class="space-y-5">
                    <!-- Nama -->
                    <div>
