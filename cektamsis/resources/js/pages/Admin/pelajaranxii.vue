...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: 'Data Absen Pelajaran Kelas XII', href: '/pelajaranx' },
];
...
const proceedDelete = () => {
  if (selectedabsensi.value) {
    window.location.href = route('pelajaranxii.hapus', selectedabsensi.value.id_absensi_pelajaran)
  }
}

// Function search
const performSearch = () => {
  router.get(route('pelajaranxii'), { search: search.value }, { preserveState: true})
}
</script>

<template>
  <Head title="Data Absen Pelajaran Kelas XII" />
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-6 overflow-x-auto">
      <div class="rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-lg">
        ...
        <div class="flex items-center justify-between mb-6">
          <h1 class="text-xl font-semibold text-white">Data Absen Pelajaran Kelas XII</h1>
        </div>
          ...
          <table class="min-w-full divide-y divide-zinc-800">
            <thead class="bg-zinc-800">
              <tr>
                ...
                <th class="px-6 py-3 text-center text-xs font-semibold uppercase text-zinc-300">Waktu Scan</th>
            ...
            <tbody class="divide-y divide-zinc-800 bg-zinc-900 text-sm text-zinc-200">
              <tr v-for="ab in props.absen.data" :key="ab.id_absensi_pelajaran"
                class="hover:bg-zinc-800/60 transition">
                <td class="px-6 py-4 text-center">{{ ab.id_absensi_pelajaran }}</td>
                <td class="px-6 py-4 text-center">{{ ab.siswa }}</td>
                <td class="px-6 py-4 text-center">{{ ab.mapel }}</td>
                <td class="px-6 py-4 text-center">{{ ab.waktu_scan }}</td>
                ...
                <td class="px-6 py-4 text-center">
                  <div class="flex justify-center gap-2">
                    <TextLink
                      :href="route('pelajaranxii.edit', ab.id_absensi_pelajaran)"
