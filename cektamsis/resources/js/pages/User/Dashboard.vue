<script setup lang="ts">
    ...
    interface Props {
        ...
        recentAttendance: Array < {
            name: string;time: string;status: string;verifikasi: string;color: string
        } > ;
        auth: {
            user: UserType
        };
        ...
        navigator.geolocation.getCurrentPosition(
            (pos) => {
                ...
                router.post(route('absen.checkout'), payload, {
                    ...
                    onError: (errors) => {
                        showNotification('❌ Gagal absen pulang! ' + (errors.error || errors
                            .keterangan || ''), 'error');
                    },
...
<template>
    ...
    <div class="min-h-screen bg-gray-50 p-6">
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
            <div
                class="rounded-2xl border border-gray-200 bg-white p-6 shadow-md hover:-translate-y-1 hover:shadow-lg transition-all duration-300">
                <div class="mb-6">
                ...
                <div class="space-y-4">
                    <div class="rounded-2xl border border-gray-200 p-4 hover:bg-gray-50">
                        ...
                        <div v-if="['izin', 'sakit'].includes(selectedStatus)" class="mb-3">
                            <label for="checkin_description"
                                class="mb-2 block text-sm font-medium text-gray-700">Keterangan</label>
                            <textarea id="checkin_description" v-model="checkinDescription"
                                ...
                                :class="{ 'border-red-500': checkinDescriptionError }" placeholder="Masukkan keterangan" required></textarea>
                        ...
                        <p class="text-sm font-medium text-gray-900">
                            ...
                            <span
                                :class="{
                                    'font-semibold text-green-600': checkinStatus === 'Sudah Absen' ||
                                        checkinStatus === 'Hadir',
                        ...
                        <button @click="checkIn"
                            ...
                            :class="!canCheckIn || checkinStatus.includes('Sudah') ?
                    ...
                    <div class="rounded-2xl border border-gray-200 p-4 hover:bg-gray-50">
                        ...
                        <p class="mb-3 text-sm font-medium text-gray-900">
                            Status: <span
                                :class="checkoutStatus === 'Sudah Pulang' ? 'text-green-600' : 'text-gray-600'">{{ checkoutStatus }}</span>
                        </p>
                    ...
                    <div class="mt-4">
                        ...
                        <div v-if="['izin', 'sakit'].includes(selectedStatus1)" class="mb-3">
                            <label for="selected_mapel" class="mb-2 block text-sm font-medium text-gray-700"> Pilih Mata
                                Pelajaran </label>
                            <select id="selected_mapel" v-model="selectedMapel"
                                class="mb-3 w-full rounded-lg border border-gray-300 p-2 text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500">
                            ...
                            <label for="checkin_description" class="mb-2 block text-sm font-medium text-gray-700">
                                Keterangan </label>
                            <textarea id="checkin_description" v-model="checkinDescription"
                                class="w-full rounded-lg border border-gray-300 p-3 text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                                :class="{ 'border-red-500': checkinDescriptionError }" placeholder="Masukkan keterangan" required></textarea>
                ...
                <div class="space-y-4">
                    <div v-for="(attendance, index) in props.recentAttendance.slice(0, 4)" :key="index"
                        ...
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <span class="text-xs font-medium rounded-full px-2 py-0.5"
                                :class="{
                                    ...
                                    'bg-orange-100 text-orange-600': attendance.color === 'orange',
                                    'bg-purple-100 text-purple-600': attendance.color === 'purple',
                        ...
                        <p v-if="['Izin', 'Sakit'].includes(attendance.status)" class="text-xs text-gray-500">
                            ...
                            <span
                                :class="{
                                    'bg-green-100 text-green-700 border border-green-300': attendance
                                        .verifikasi === 'approve',
                                    'bg-red-100 text-red-700 border border-red-300': attendance
                                        .verifikasi === 'disapprove',
                                    'bg-yellow-100 text-yellow-700 border border-yellow-300': attendance
                    ...
                    <p v-if="props.recentAttendance.length === 0" class="text-center text-gray-500 text-sm">
        ...
        <div v-if="showWeekendModal"
            ...
            <div class="w-full max-w-md transform overflow-hidden rounded-2xl bg-white shadow-2xl" @click.stop>
                <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                    ...
                    <button @click="closeWeekendModal" type="button"
                ...
                <div class="space-y-4 p-6">
                    ...
                    <button @click="closeWeekendModal"
        ...
        <div v-if="showEarlyCheckoutModal"
            class="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm"
            ...
            <div class="w-full max-w-md transform overflow-hidden rounded-2xl bg-white shadow-2xl" @click.stop>
                <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                    ...
                    <button @click="closeEarlyCheckoutModal" type="button"
                        class="text-gray-400 transition-colors hover:text-gray-600">✕</button>
                </div>
                <div class="space-y-4 p-6">
                    <div>
                        <label for="checkout_description"
                            class="mb-2 block text-sm font-medium text-gray-700">Keterangan Pulang Cepat</label>
                        <textarea id="checkout_description" v-model="checkoutDescription"
                            ...
                            :class="{ 'border-red-500': checkoutDescriptionError }" placeholder="Masukkan alasan pulang cepat" required></textarea>
                        <p v-if="checkoutDescriptionError" class="mt-1 text-sm text-red-500">
                            {{ checkoutDescriptionError }}</p>
                    </div>
                    <div class="flex gap-3 pt-4">
                        <button type="button" @click="closeEarlyCheckoutModal"
                            class="flex-1 rounded-xl bg-gray-500 py-3 font-medium text-white transition-colors hover:bg-gray-600">
                            Batal
                        </button>
                        <button type="button" @click="performCheckout" :disabled="processingOut"
        ...
        <div v-if="showChangePasswordModal"
            ...
            <div class="w-full max-w-md transform overflow-hidden rounded-2xl bg-white shadow-2xl" @click.stop>
                ...
                <form @submit.prevent="submitPasswordChange" class="space-y-4 p-6">
                    <div>
                        <label for="current_password" class="mb-2 block text-sm font-medium text-gray-700">Password
                            Saat Ini</label>
                    ...
                    <div>
                        <label for="new_password" class="mb-2 block text-sm font-medium text-gray-700">Password
                            Baru</label>
                        <input id="new_password" v-model="passwordForm.new_password" type="password"
                            class="w-full rounded-lg border border-gray-300 p-3 text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                    ...
                    <div>
                        <label for="new_password_confirmation"
                            class="mb-2 block text-sm font-medium text-gray-700">Konfirmasi Password Baru</label>
